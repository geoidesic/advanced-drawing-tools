...
Hooks.on("canvasInit", () => {
    ...
    foundry.canvas.containers.PreciseText._MAX_TEXTURE_SIZE = gl.getParameter(gl.MAX_TEXTURE_SIZE);
    foundry.canvas.containers.PreciseText._MAX_ZOOM = Math.min(CONFIG.Canvas.maxZoom, 4);

    if (canvas.performance.mode <= CONST.CANVAS_PERFORMANCE_MODES.MED) {
        foundry.canvas.containers.PreciseText._MAX_RESOLUTION = 2;
    } else if (canvas.performance.mode <= CONST.CANVAS_PERFORMANCE_MODES.HIGH) {
        foundry.canvas.containers.PreciseText._MAX_RESOLUTION = foundry.canvas.containers.PreciseText._MAX_ZOOM > 3 ? 3 : 2;
    } else {
        foundry.canvas.containers.PreciseText._MAX_RESOLUTION = foundry.canvas.containers.PreciseText._MAX_ZOOM;
    }
});
